...
See tests/core/jmap/test_client.py's module docstring for why this is
a NotImplementedError-catching test rather than xfail: wait() would
block on a live JMAP EventSource connection, which isn't something
this environment can exercise honestly. Reconnect/backoff scheduling
itself is already implemented and tested separately
(spork.core.jmap.backoff) — what's missing here is just the actual
listen loop.
"""
