"""UNIT_FILE_CONTENT: the sporkd systemd unit template, as a string (docs/DESIGN.md §14).
...
# The systemd ExecStart must remain one physical unit-file line.
# ruff: noqa: E501
UNIT_FILE_CONTENT = """\
[Unit]
...
[Service]
Type=notify
ExecStartPre=%h/.local/bin/spork --secretspec %h/.config/spork/%i/secretspec.toml secrets wait --timeout 120
