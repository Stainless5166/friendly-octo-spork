...
def install_service_command(
    instance: str = typer.Argument(
        "default", help="Instance name to enable and start (for example, personal)."
    ),
    ...
    `systemctl --user enable --now sporkd@<instance>`. `loginctl enable-linger
    ...
    """
    try:
        path = install_service(instance=instance, enable_now=enable_now)
    ...
    if enable_now:
        typer.echo(f"Enabled and started sporkd@{instance} (systemctl --user enable --now)")
    else:
        typer.echo(f"Run `systemctl --user enable --now sporkd@{instance}` to enable and start it.")
